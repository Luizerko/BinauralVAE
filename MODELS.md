...
$$\pmb{S} = \begin{bmatrix} \sigma_{xx} & \sigma_{xy} \\\\ \sigma_{yx} & \sigma_{yy} \end{bmatrix} = \begin{bmatrix} \frac{\sigma + \delta_r}{2} & \frac{\delta_i}{2} \\\\ \frac{\delta_i}{2} & \frac{\sigma - \delta_r}{2} \end{bmatrix}$$

### CVAE Formulation and KL-Divergence

So now that we understand complex Gaussian distributions a bit better, let’s get to the actual formulation of the CVAE. Let’s first talk about the prior. For the standard VAE, the prior $p(\pmb{h})$ is a standard normal $\mathcal{N}(\pmb{0}, \pmb{I})$. For the CVAE, we’ll also have a standard complex normal $\mathcal{N}_c(\pmb{0}, \pmb{I}, \pmb{O})$, with the last parameter being a matrix full of zeros. This means there’s no correlation at all between complex variables, and every single one of them is a perfect unit circle distribution.

Now for the posterior. In the standard VAE, we have $q_{\phi}(\pmb{h}|\pmb{z}) = \mathcal{N}(\pmb{\mu}, \Delta(\pmb{\sigma}))$, with $\Delta$ indicating a diagonal matrix. This basically tells us that we have the freedom to expand variables individually, but not skew the distribution. This is the case for standard VAEs for two reasons: first, it makes the math tractable, and second, it actually gives every latent dimension a unique meaning - we don’t allow latent dimensions to correlate because we want them to express their individual information. For the exact same reasons, we’ll follow an analogous approach for the complex posterior, defining it as $q_{\phi}(\pmb{h}|\pmb{z}) = \mathcal{N}_c(\pmb{\mu}, \Delta(\pmb{\sigma}), \Delta(\pmb{\delta}))$. This means that we allow the complex variables of the posterior to have no correlation between one another, but we allow each individual variable to be an ellipse (and even skewed) within its own real/imaginary parts, and with arbitrary (positive) size.

With this formulation, we can derive the KL-Divergence. Because all the complex variables are independent of one another (thanks to the diagonal matrices), the total KL-Divergence is just the sum of the divergences of each individual latent variable $h_j$. We can compute the divergence for each $h_j$ by treating its prior and posterior as bivariate (2D) real Gaussians. The general KL-Divergence formula between a posterior $q = \mathcal{N}(\pmb{\mu}_q, \pmb{\Sigma}_q)$ and a prior $p = \mathcal{N}(\pmb{\mu}_p, \pmb{\Sigma}_p)$ for a $k$-dimensional distribution is:

$$D_{KL}(q || p) = \frac{1}{2} \left[ \text{tr}(\pmb{\Sigma}_{p}^{-1} \pmb{\Sigma}_{q}) + (\pmb{\mu}_q - \pmb{\mu}_p)^T \pmb{\Sigma}_{p}^{-1} (\pmb{\mu}_q - \pmb{\mu}_p) - k + \ln\left(\frac{|\pmb{\Sigma}_{p}|}{|\pmb{\Sigma}_{q}|}\right) \right]$$

Now we plug in the values for a single complex variable $h_j$ (where $k=2$ dimensions, real and imaginary):

- The prior ($p$) is a standard complex normal, meaning the total variance is 1, split equally between the real and imaginary parts. So, $\pmb{\mu}_p = \pmb{0}$ and $\pmb{\Sigma}_p = \frac{1}{2}\pmb{I}$. This makes its inverse $\pmb{\Sigma}_p^{-1} = 2\pmb{I}$ and its determinant is $|\pmb{\Sigma}_p| = \frac{1}{4}$.

- The posterior ($q$) has mean $\pmb{\mu}_q = [\text{Re}(\mu_j), \text{Im}(\mu_j)]^T$ and, from our previous section, a $2 \times 2$ covariance matrix given by $\pmb{\Sigma}_q = \pmb{S}_j = \begin{bmatrix} \frac{\sigma_j + \text{Re}(\delta_j)}{2} & \frac{\text{Im}(\delta_j)}{2} \\\\ \frac{\text{Im}(\delta_j)}{2} & \frac{\sigma_j - \text{Re}(\delta_j)}{2} \end{bmatrix}$. Its determinant is $|\pmb{\Sigma}_q| = \frac{\sigma_j^2 - |\delta_j|^2}{4}$.

Now, we evaluate each term inside the KL-Divergence brackets:

1.  $\text{tr}(2\pmb{I} \cdot \pmb{\Sigma}_q) = 2 \left( \frac{\sigma_j + \text{Re}(\delta_j)}{2} + \frac{\sigma_j - \text{Re}(\delta_j)}{2} \right) = 2\sigma_j$

2.  $\pmb{\mu}_q^T (2\pmb{I}) \pmb{\mu}_q = 2 (\text{Re}(\mu_j)^2 + \text{Im}(\mu_j)^2) = 2|\mu_j|^2$
...
$$D_{KL}(q_{\phi}(\pmb{h}|\pmb{z}) || p(\pmb{h})) = \pmb{\mu}^H\pmb{\mu} + \left|\left| \pmb{\sigma} - \pmb{1} - \frac{1}{2}\log(\pmb{\sigma}^2 - |\pmb{\delta}|^2) \right|\right|_1$$

### The Reparameterization Trick

Even though we have the KL-Divergence, we still need to handle the reparameterization trick to be able to sample from the posterior distribution while also allowing gradients to flow. This is a bit more tricky than the standard VAE, but we’ll get there. Let’s first decompose the latent variables $\pmb{h}$ into real and imaginary parts $\pmb{x} \in \mathbb{R}^N$ and $\pmb{y} \in \mathbb{R}^N$ as $\pmb{h} = \pmb{x} + i\pmb{y}$. Under the assumption of the diagonal covariance and pseudo-covariance matrices, which is the posterior distribution we are actually sampling our latent variables from, we know that all the complex variables are independent from one another and we can understand each individual $\pmb{h}_j \in \pmb{h}$ as a 2D real Gaussian distribution. These variables, as discussed before, are $\pmb{h}_j = [x, y]^T$ and follow $\pmb{h}_j \sim \mathcal{N}(\pmb{\mu}_{h_j}, \pmb{\Sigma}_{h_j})$.

For the reparameterization trick, we need to find a transformation matrix $\pmb{L}$ such that if we sample two independent standard normal variables $\pmb{\epsilon} = [\epsilon_r, \epsilon_i]^T \sim \mathcal{N}(\pmb{0}, \pmb{I})$, then $\pmb{h}_j = \pmb{\mu_{h_j}} + \pmb{L}\pmb{\epsilon}$ produces the covariance $\pmb{\Sigma_{h_j}} = \pmb{L}\pmb{L}^T$. To understand that better, think about it like this: if we sample a vector $\pmb{\epsilon} \sim \mathcal{N}(\pmb{0}, \pmb{I})$ and want to stretch it using some matrix $\pmb{A}$, we do $\pmb{z} = \pmb{A}\pmb{\epsilon}$. Now the covariance of this new vector is $Cov(\pmb{A}\pmb{\epsilon}) = \pmb{A} \cdot Cov(\pmb{\epsilon}) \cdot \pmb{A}^T$. Because $Cov(\pmb{\epsilon}) = \pmb{I}$, this simplifies to $Cov(\pmb{z}) = \pmb{A} \pmb{I} \pmb{A}^T = \pmb{A}\pmb{A}^T$. Now, if we want our new covariance matrix to be $\pmb{S}$ (our $\pmb{\Sigma_{h_j}}$) we have to solve $\pmb{A}\pmb{A}^T = \pmb{S}$.

This is solved using the Cholesky decomposition, which is the matrix equivalent of finding a square root. Because $\pmb{L}$ must be a lower-triangular matrix, we set it up as 

$$\pmb{L} = \begin{bmatrix} l_{11} & 0 \\\\ l_{21} & l_{22} \end{bmatrix}$$

Multiplying $\pmb{L}$ by its transpose $\pmb{L}^T$ gives us:

$$\pmb{L}\pmb{L}^T = \begin{bmatrix} l_{11} & 0 \\\\ l_{21} & l_{22} \end{bmatrix} \begin{bmatrix} l_{11} & l_{21} \\\\ 0 & l_{22} \end{bmatrix} = \begin{bmatrix} l_{11}^2 & l_{11}l_{21} \\\\ l_{11}l_{21} & l_{21}^2 + l_{22}^2 \end{bmatrix}$$

We set this equal to our target covariance matrix $\pmb{\Sigma_{h_j}}$, which we mapped out using $\sigma$ and $\delta$ in a previous section to get:
...
Now we simply solve this system of equations to find the values inside $\pmb{L}$:
...
We now multiply our standard normal noise by $\pmb{L}$ to get the exact covariance shape $\pmb{\Sigma_{h_j}}$: 

$$\begin{bmatrix} x_{shifted} \\\\ y_{shifted} \end{bmatrix} = \pmb{L} \begin{bmatrix} \epsilon_r \\\\ \epsilon_i \end{bmatrix} = \begin{bmatrix} l_{11} & 0 \\\\ l_{21} & l_{22} \end{bmatrix} \begin{bmatrix} \epsilon_r \\\\ \epsilon_i \end{bmatrix}$$

Notice that $x_{shifted} = l_{11}\epsilon_r$. This means that, because $\pmb{L}$ is lower triangular, the real part $x$ only depends on $\epsilon_r$. But $y_{shifted} = l_{21}\epsilon_r + l_{22}\epsilon_i$, so the imaginary part $y$ depends on both. This is what creates the covariance and correlation between them. Now we just add our mean vector $\pmb{\mu_{h_j}} = [\mu_r, \mu_i]^T$ to get the final, explicit coordinates for our sampled complex number: $x = \mu_r + l_{11}\epsilon_r$ and $y = \mu_i + l_{21}\epsilon_r + l_{22}\epsilon_i$. By factoring out the $\epsilon_r$ and $\epsilon_i$, we get Nakashika's complex multipliers $k_r$ and $k_i$.
...
And finally in the vector form, we have $\pmb{\tilde{h}} = \pmb{\mu} + \pmb{k}_r \pmb{\epsilon}_r + \pmb{k}_i \pmb{\epsilon}_i$.

### Reconstruction Loss

After passing through the complex latent space, we require a maximum likelihood estimation of our data $\pmb{z}$ given the latent variables $\pmb{h}$. Assuming our output is a complex normal $\mathcal{N}_c(\pmb{\mu}, \pmb{I}, \pmb{O})$, this approximately simplifies to the MSE in exact parallel to a standard VAE. The probability density function for a circular complex Gaussian of a $D$-dimensional vector $\pmb{z}$ is defined as: $p(\pmb{z}) = \frac{1}{\pi^D \vert{}\pmb{\Gamma}\vert{}} \exp\left( -(\pmb{z} - \pmb{\mu})^H \pmb{\Gamma}^{-1} (\pmb{z} - \pmb{\mu}) \right)$. Because we defined our covariance $\pmb{\Gamma}$ as the identity matrix $\pmb{I}$, the equation simplifies to $p_\theta(\pmb{z}\vert{}\pmb{h}) = \frac{1}{\pi^D} \exp\left( -(\pmb{z} - \pmb{\mu})^H (\pmb{z} - \pmb{\mu}) \right)$. Now, to get our log-likelihood, we just take the natural logarithm of both sides:

$$\log p_\theta(\pmb{z}\vert{}\pmb{h}) = \log\left( \frac{1}{\pi^D} \right) - (\pmb{z} - \pmb{\mu})^H (\pmb{z} - \pmb{\mu})$$
$$\log p_\theta(\pmb{z}\vert{}\pmb{h}) = -D\log(\pi) - (\pmb{z} - \pmb{\mu})^H (\pmb{z} - \pmb{\mu})$$

But when optimizing a neural network, we only care about terms affected by our network's weights. The term $-D\log(\pi)$ is a constant, meaning its gradient is zero, so we can drop it from our loss function. We also know that multiplying a complex vector by its conjugate transpose yields the squared $L_2$ norm, therefore we have $\log p_\theta(\pmb{z}\vert{}\pmb{h}) \propto -\vert{}\vert{}\pmb{z} - \pmb{\mu}\vert{}\vert{}_2^2$. Now when we take the expectation $\mathbb{E}_{q_\phi}$ over our sampled latent variables, we also stick to the one sample Monte Carlo on top of the reparameterization trick, so we get $ \mathbb{E}_{q_\phi(\pmb{h}|\pmb{z})}[\log p_\theta(\pmb{z}|\pmb{h})] \approx -||\pmb{z} - \pmb{\mu}||_2^2 $ and we can simply calculate this squared distance.
