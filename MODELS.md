...
Now, if we scale this intuition down to a single zero-mean complex variable $z = x + iy$, we can clearly see how the diagonal elements of these matrices are formed. **Variance**: $\sigma = \mathbb{E}[zz^{\ast}] = \mathbb{E}[(x+iy)(x-iy)] = \mathbb{E}[x^2 + y^2] = \sigma_{xx} + \sigma_{yy}$. This yields a real, positive number representing the total spread. **Pseudo-Variance**: $\delta = \mathbb{E}[zz] = \mathbb{E}[(x+iy)(x+iy)] = \mathbb{E}[x^2 - y^2 + 2ixy] = (\sigma_{xx} - \sigma_{yy}) + 2i\sigma_{xy}$. This yields a complex number ($\delta_r + i\delta_i$) that captures the imbalance between the real and imaginary variances ($\sigma_{xx} - \sigma_{yy}$) and their correlation ($\sigma_{xy}$), defining how elliptical (and skewed) the distribution is.
...
$$\boldsymbol{S} = \begin{bmatrix} \sigma_{xx} & \sigma_{xy} \\\\ \sigma_{yx} & \sigma_{yy} \end{bmatrix} = \begin{bmatrix} \frac{\sigma + \delta_r}{2} & \frac{\delta_i}{2} \\\\ \frac{\delta_i}{2} & \frac{\sigma - \delta_r}{2} \end{bmatrix}$$

### CVAE Formulation and KL-Divergence

So now that we understand complex Gaussian distributions a bit better, let’s get to the actual formulation of the CVAE. Let’s first talk about the prior. For the standard VAE, the prior $p(\boldsymbol{h})$ is a standard normal $\mathcal{N}(\boldsymbol{0}, \boldsymbol{I})$. For the CVAE, we’ll also have a standard complex normal $\mathcal{N}_c(\boldsymbol{0}, \boldsymbol{I}, \boldsymbol{O})$, with the last parameter being a matrix full of zeros. This means there’s no correlation at all between complex variables, and every single one of them is a perfect unit circle distribution.

Now for the posterior. In the standard VAE, we have $q_{\phi}(\boldsymbol{h}|\boldsymbol{z}) = \mathcal{N}(\boldsymbol{\mu}, \Delta(\boldsymbol{\sigma}))$, with $\Delta$ indicating a diagonal matrix. This basically tells us that we have the freedom to expand variables individually, but not skew the distribution. This is the case for standard VAEs for two reasons: first, it makes the math tractable, and second, it actually gives every latent dimension a unique meaning - we don’t allow latent dimensions to correlate because we want them to express their individual information. For the exact same reasons, we’ll follow an analogous approach for the complex posterior, defining it as $q_{\phi}(\boldsymbol{h}|\boldsymbol{z}) = \mathcal{N}_c(\boldsymbol{\mu}, \Delta(\boldsymbol{\sigma}), \Delta(\boldsymbol{\delta}))$. This means that we allow the complex variables of the posterior to have no correlation between one another, but we allow each individual variable to be an ellipse (and even skewed) within its own real/imaginary parts, and with arbitrary (positive) size.

With this formulation, we can derive the KL-Divergence. Because all the complex variables are independent of one another (thanks to the diagonal matrices), the total KL-Divergence is just the sum of the divergences of each individual latent variable $h_j$. We can compute the divergence for each $h_j$ by treating its prior and posterior as bivariate (2D) real Gaussians. The general KL-Divergence formula between a posterior $q = \mathcal{N}(\boldsymbol{\mu}_q, \boldsymbol{\Sigma}_q)$ and a prior $p = \mathcal{N}(\boldsymbol{\mu}_p, \boldsymbol{\Sigma}_p)$ for a $k$-dimensional distribution is:

$$D_{KL}(q || p) = \frac{1}{2} \left[ \text{tr}(\boldsymbol{\Sigma}_{p}^{-1} \boldsymbol{\Sigma}_{q}) + (\boldsymbol{\mu}_q - \boldsymbol{\mu}_p)^T \boldsymbol{\Sigma}_{p}^{-1} (\boldsymbol{\mu}_q - \boldsymbol{\mu}_p) - k + \ln\left(\frac{|\boldsymbol{\Sigma}_{p}|}{|\boldsymbol{\Sigma}_{q}|}\right) \right]$$

Now we plug in the values for a single complex variable $h_j$ (where $k=2$ dimensions, real and imaginary):

- The prior ($p$) is a standard complex normal, meaning the total variance is 1, split equally between the real and imaginary parts. So, $\boldsymbol{\mu}_p = \boldsymbol{0}$ and $\boldsymbol{\Sigma}_p = \frac{1}{2}\boldsymbol{I}$. This makes its inverse $\boldsymbol{\Sigma}_p^{-1} = 2\boldsymbol{I}$ and its determinant is $|\boldsymbol{\Sigma}_p| = \frac{1}{4}$.

- The posterior ($q$) has mean $\boldsymbol{\mu}_q = [\text{Re}(\mu_j), \text{Im}(\mu_j)]^T$ and, from our previous section, a $2 \times 2$ covariance matrix given by $\boldsymbol{\Sigma}_q = \boldsymbol{S}_j = \begin{bmatrix} \frac{\sigma_j + \text{Re}(\delta_j)}{2} & \frac{\text{Im}(\delta_j)}{2} \\\\ \frac{\text{Im}(\delta_j)}{2} & \frac{\sigma_j - \text{Re}(\delta_j)}{2} \end{bmatrix}$. Its determinant is $|\boldsymbol{\Sigma}_q| = \frac{\sigma_j^2 - |\delta_j|^2}{4}$.

Now, we evaluate each term inside the KL-Divergence brackets:

1.  $\text{tr}(2\boldsymbol{I} \cdot \boldsymbol{\Sigma}_q) = 2 \left( \frac{\sigma_j + \text{Re}(\delta_j)}{2} + \frac{\sigma_j - \text{Re}(\delta_j)}{2} \right) = 2\sigma_j$

2.  $\boldsymbol{\mu}_q^T (2\boldsymbol{I}) \boldsymbol{\mu}_q = 2 (\text{Re}(\mu_j)^2 + \text{Im}(\mu_j)^2) = 2|\mu_j|^2$
...
$$D_{KL}(q_{\phi}(\boldsymbol{h}|\boldsymbol{z}) || p(\boldsymbol{h})) = \boldsymbol{\mu}^H\boldsymbol{\mu} + \left|\left| \boldsymbol{\sigma} - \boldsymbol{1} - \frac{1}{2}\log(\boldsymbol{\sigma}^2 - |\boldsymbol{\delta}|^2) \right|\right|_1$$

### The Reparameterization Trick

Even though we have the KL-Divergence, we still need to handle the reparameterization trick to be able to sample from the posterior distribution while also allowing gradients to flow. This is a bit more tricky than the standard VAE, but we’ll get there. Let’s first decompose the latent variables $\boldsymbol{h}$ into real and imaginary parts $\boldsymbol{x} \in \mathbb{R}^N$ and $\boldsymbol{y} \in \mathbb{R}^N$ as $\boldsymbol{h} = \boldsymbol{x} + i\boldsymbol{y}$. Under the assumption of the diagonal covariance and pseudo-covariance matrices, which is the posterior distribution we are actually sampling our latent variables from, we know that all the complex variables are independent from one another and we can understand each individual $\boldsymbol{h}_j \in \boldsymbol{h}$ as a 2D real Gaussian distribution. These variables, as dicussed before, are $\boldsymbol{h}_j = [x, y]^T$ and follow $\boldsymbol{h}_j \sim \mathcal{N}(\boldsymbol{\mu_{h_j}}, \boldsymbol{\Sigma_{h_j}})$.

For the reparameterization trick, we need to find a transformation matrix $\boldsymbol{L}$ such that if we sample two independent standard normal variables $\boldsymbol{\epsilon} = [\epsilon_r, \epsilon_i]^T \sim \mathcal{N}(\boldsymbol{0}, \boldsymbol{I})$, then $\boldsymbol{h}_j = \boldsymbol{\mu_{h_j}} + \boldsymbol{L}\boldsymbol{\epsilon}$ produces the covariance $\boldsymbol{\Sigma_{h_j}} = \boldsymbol{L}\boldsymbol{L}^T$. To understand that better, think about it like this: if we sample a vector $\boldsymbol{\epsilon} \sim \mathcal{N}(\boldsymbol{0}, \boldsymbol{I})$ and want to stretch it using some matrix $\boldsymbol{A}$, we do $\boldsymbol{z} = \boldsymbol{A}\boldsymbol{\epsilon}$. Now the covariance of this new vector is $Cov(\boldsymbol{A}\boldsymbol{\epsilon}) = \boldsymbol{A} \cdot Cov(\boldsymbol{\epsilon}) \cdot \boldsymbol{A}^T$. Because $Cov(\boldsymbol{\epsilon}) = \boldsymbol{I}$, this simplifies to $Cov(\boldsymbol{z}) = \boldsymbol{A} \boldsymbol{I} \boldsymbol{A}^T = \boldsymbol{A}\boldsymbol{A}^T$. Now, if we want our new covariance matrix to be $\boldsymbol{S}$ (our $\boldsymbol{\Sigma_{h_j}}$) we have to solve $\boldsymbol{A}\boldsymbol{A}^T = \boldsymbol{S}$.

This is solved using the Cholesky decomposition, which is the matrix equivalent of finding a square root. Because $\boldsymbol{L}$ must be a lower-triangular matrix, we set it up as $\boldsymbol{L} = \begin{bmatrix} l_{11} & 0 \\\\ l_{21} & l_{22} \end{bmatrix}$. Multiplying $\boldsymbol{L}$ by its transpose $\boldsymbol{L}^T$ gives us:

$$\boldsymbol{L}\boldsymbol{L}^T = \begin{bmatrix} l_{11} & 0 \\\\ l_{21} & l_{22} \end{bmatrix} \begin{bmatrix} l_{11} & l_{21} \\\\ 0 & l_{22} \end{bmatrix} = \begin{bmatrix} l_{11}^2 & l_{11}l_{21} \\\\ l_{11}l_{21} & l_{21}^2 + l_{22}^2 \end{bmatrix}$$

We set this equal to our target covariance matrix $\boldsymbol{\Sigma_{h_j}}$, which we mapped out using $\sigma$ and $\delta$ in a previous section to get:
...
Now we simply solve this system of equations to find the values inside $\boldsymbol{L}$:
...
We now multiply our standard normal noise by $\boldsymbol{L}$ to get the exact covariance shape $\boldsymbol{\Sigma_{h_j}}$: 

$$\begin{bmatrix} x_{shifted} \\\\ y_{shifted} \end{bmatrix} = \boldsymbol{L} \begin{bmatrix} \epsilon_r \\\\ \epsilon_i \end{bmatrix} = \begin{bmatrix} l_{11} & 0 \\\\ l_{21} & l_{22} \end{bmatrix} \begin{bmatrix} \epsilon_r \\\\ \epsilon_i \end{bmatrix}$$

Notice that $x_{shifted} = l_{11}\epsilon_r$. This means that, because $\boldsymbol{L}$ is lower triangular, the real part $x$ only depends on $\epsilon_r$. But $y_{shifted} = l_{21}\epsilon_r + l_{22}\epsilon_i$, so the imaginary part $y$ depends on both. This is what creates the covariance and correlation between them. Now we just add our mean vector $\boldsymbol{\mu_{h_j}} = [\mu_r, \mu_i]^T$ to get the final, explicit coordinates for our sampled complex number: $x = \mu_r + l_{11}\epsilon_r$ and $y = \mu_i + l_{21}\epsilon_r + l_{22}\epsilon_i$. By factoring out the $\epsilon_r$ and $\epsilon_i$, we get Nakashika's complex multipliers $k_r$ and $k_i$.
...
And finally in the vector form, we have $\boldsymbol{\tilde{h}} = \boldsymbol{\mu} + \boldsymbol{k}_r \boldsymbol{\epsilon}_r + \boldsymbol{k}_i \boldsymbol{\epsilon}_i$.

### Reconstruction Loss

After passing through the complex latent space, we require a maximum likelihood estimation of our data $\boldsymbol{z}$ given the latent variables $\boldsymbol{h}$. Assuming our output is a complex normal $\mathcal{N}_c(\boldsymbol{\mu}, \boldsymbol{I}, \boldsymbol{O})$, this approximately simplifies to the MSE in exact parallel to a standard VAE. The probability density function for a circular complex Gaussian of a $D$-dimensional vector $\boldsymbol{z}$ is defined as: $p(\boldsymbol{z}) = \frac{1}{\pi^D \vert{}\boldsymbol{\Gamma}\vert{}} \exp\left( -(\boldsymbol{z} - \boldsymbol{\mu})^H \boldsymbol{\Gamma}^{-1} (\boldsymbol{z} - \boldsymbol{\mu}) \right)$. Because we defined our covariance $\boldsymbol{\Gamma}$ as the identity matrix $\boldsymbol{I}$, the equation simplifies to $p_\theta(\boldsymbol{z}\vert{}\boldsymbol{h}) = \frac{1}{\pi^D} \exp\left( -(\boldsymbol{z} - \boldsymbol{\mu})^H (\boldsymbol{z} - \boldsymbol{\mu}) \right)$. Now, to get our log-likelihood, we just take the natural logarithm of both sides:

$$\log p_\theta(\boldsymbol{z}\vert{}\boldsymbol{h}) = \log\left( \frac{1}{\pi^D} \right) - (\boldsymbol{z} - \boldsymbol{\mu})^H (\boldsymbol{z} - \boldsymbol{\mu})$$
$$\log p_\theta(\boldsymbol{z}\vert{}\boldsymbol{h}) = -D\log(\pi) - (\boldsymbol{z} - \boldsymbol{\mu})^H (\boldsymbol{z} - \boldsymbol{\mu})$$

But when optimizing a neural network, we only care about terms affected by our network's weights. The term $-D\log(\pi)$ is a constant, meaning its gradient is zero, so we can drop it from our loss function. We also know that multiplying a complex vector by its conjugate transpose yields the squared $L_2$ norm, therefore we have $\log p_\theta(\boldsymbol{z}\vert{}\boldsymbol{h}) \propto -\vert{}\vert{}\boldsymbol{z} - \boldsymbol{\mu}\vert{}\vert{}_2^2$. Now when we take the expectation $\mathbb{E}_{q_\phi}$ over our sampled latent variables, we also stick to the one sample Monte Carlo on top of the reparameterization trick, so we get $ \mathbb{E}_{q_\phi(\boldsymbol{h}|\boldsymbol{z})}[\log p_\theta(\boldsymbol{z}|\boldsymbol{h})] \approx -||\boldsymbol{z} - \boldsymbol{\mu}||_2^2 $ and we can simply calculate this squared distance.
