...
This is solved using the Cholesky decomposition, which is the matrix equivalent of finding a square root. Because $\boldsymbol{L}$ must be a lower-triangular matrix, we set it up as 

$$\boldsymbol{L} = \begin{bmatrix} l_{11} & 0 \\\\ l_{21} & l_{22} \end{bmatrix}$$

Multiplying $\boldsymbol{L}$ by its transpose $\boldsymbol{L}^T$ gives us:
